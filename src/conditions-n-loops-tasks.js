...
function isContainNumber(num, digit) {
  let numLink = num;
  while (numLink > 0) {
    const lastDigit = numLink % 10;
    if (lastDigit === digit) return true;
    numLink = Math.floor(numLink / 10);
  }
  return false;
}
...
function rotateMatrix(matrix) {
  ...
  for (let i = 0; i < n; i += 1) {
    for (let j = i + 1; j < n; j += 1) {
      const temp = arrayLink[i][j];
      arrayLink[i][j] = arrayLink[j][i];
      arrayLink[j][i] = temp;
    }
  }
  ...
  for (let i = 0; i < n; i += 1) {
    ...
    let right = n - 1;
    while (left < right) {
      const temp = arrayLink[i][left];
      arrayLink[i][left] = arrayLink[i][right];
      arrayLink[i][right] = temp;
      left += 1;
      right -= 1;
    }
  }
...
function sortByAsc(arr) {
  const arrayLink = arr;
  for (let i = 0; i < arrayLink.length; i += 1) {
    const currentElement = arrayLink[i];
    let j = i - 1;
    while (j >= 0 && arrayLink[j] > currentElement) {
      arrayLink[j + 1] = arrayLink[j];
      j -= 1;
    }
    arrayLink[j + 1] = currentElement;
  }
  return arr;
}
...
function shuffleChar(str, iterations) {
  ...
  for (let k = 1; k <= iterations; k += 1) {
    ...
    if (currentStr === str) {
      const cycleLength = k;
      const targetIterInCycle = iterations % cycleLength;
      return history[targetIterInCycle];
    }
    history[k] = currentStr;
  }
  return currentStr;
}
